...
@Config
@TeleOp
public class testeMainSolo extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            ...
            telemetry.addData("Eixo X", eixox.getCurrentPosition());
            telemetry.addData("Eixo Y", eixoy.getCurrentPosition());
...
//        }
    ...
    private void applyKitPIDF() {
        if (!isManualControl) {
            double RT = 1.0;
            if(Collect){
                RT = 0.5 + (gamepad1.right_trigger * 0.5);
            }
            // Aplicar PIDF apenas se o controle manual não estiver ativo
